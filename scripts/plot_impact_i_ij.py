"""Plot component composition on practically boosted I_ij edges.

Reads ``impact_i_ij_per_celltype.csv`` from ``analyze_impact_i_ij.py`` and
writes a horizontal three-panel bar plot.  The three panels show the mean
per-cell percent contribution of d_i,j^w, r_i,j, and t_i,j
on selected edges.  Cell-type labels are shown only once on the shared y-axis.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data-dir",
        default="output/weight_decomposition_retina/impact_i_ij",
        help="Directory containing impact_i_ij_per_celltype.csv.",
    )
...
def main() -> int:
    args = parse_args()
    csv_path = Path(args.csv) if args.csv else Path(args.data_dir) / "impact_i_ij_per_celltype.csv"
    if not csv_path.exists():
        raise FileNotFoundError(f"{csv_path} not found. Run analyze_impact_i_ij.py first.")
    ...
    fig.supxlabel("Mean contribution on selected edges (%)", y=0.04, fontsize=10.2)

    fig.subplots_adjust(left=left, right=0.955, top=0.90, bottom=bottom)
    png_path = out_dir / "impact_i_ij_plot.png"
    pdf_path = out_dir / "impact_i_ij_plot.pdf"
